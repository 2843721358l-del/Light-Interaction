...
This anonymized repository provides the Light Interaction framework with tested model-specific support for **HY-WorldPlay** and **Matrix-Game-3.0**, along with reproducible setup scripts, fixed-prompt generation helpers, and evaluation utilities.
...
- 📄 [2026/05] Anonymous manuscript prepared for review.
...
```bash
git clone <anonymous-repository-url> Light-Interaction
cd Light-Interaction
```
...
See the anonymous manuscript for full ablations and metric definitions.
...
Citation information is withheld during double-blind review and will be added after the review period.
